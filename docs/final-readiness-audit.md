...
- Evidence notes: the dry-run used the existing long-lived goal PR branch
  ...
  than a failure.
- PR update note: the automation attempted to refresh the existing long-lived PR
  body, but the PR update tool was bound to the initial per-run branch
  `cursor/cursor-pr-agent-engine-5f35` instead of this long-lived goal branch.
  The branch update was pushed instead, and no replacement PR was created.
...
- PR update note: the automation attempted to update the existing long-lived PR
  body, but the PR update tool was bound to the initial per-run branch
  `cursor/cursor-pr-agent-engine-d39c` instead of this long-lived goal branch.
  The branch update was pushed instead, and no replacement PR was created.
- Release policy: no auto-merge, no tag creation, and no release publication
  were performed.
